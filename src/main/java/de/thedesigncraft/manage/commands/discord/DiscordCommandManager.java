package de.thedesigncraft.manage.commands.discord;

import de.thedesigncraft.manage.commands.discord.events.DBSMessageCommandInteractionEvent;
import de.thedesigncraft.manage.commands.discord.events.DBSSlashCommandInteractionEvent;
import de.thedesigncraft.manage.commands.discord.events.DBSUserCommandInteractionEvent;
import de.thedesigncraft.manage.commands.discord.types.slashanduser.ISlashAndUserCommand;
...
import de.thedesigncraft.manage.commands.discord.types.message.IMessageContextMenu;
import de.thedesigncraft.manage.commands.discord.types.user.IUserContextMenu;
import de.thedesigncraft.manage.commands.discord.types.slash.ISlashCommand;
...
public class DiscordCommandManager extends ListenerAdapter {
    ...
    public void onSlashCommandInteraction(@NotNull SlashCommandInteractionEvent event) {
        ...
        if ((command = ISlashCommand.getByName(event.getName())) != null) {
            if (Checks.isNullOrEmpty(command.getSetup().getRequiredPermissions()) && !event.isFromGuild()) {
                command.execute(new DBSSlashCommandInteractionEvent(command, event));
                return;
            }
    ...
    public void onUserContextInteraction(@NotNull UserContextInteractionEvent event) {
        IUserContextMenu command;
        if ((command = IUserContextMenu.getByName(event.getName())) != null) {
            if (Checks.isNullOrEmpty(command.getSetup().getRequiredPermissions()) || !event.isFromGuild()) {
                command.execute(new DBSUserCommandInteractionEvent(command, event));
                return;
            }
            if (event.getMember().hasPermission(command.getSetup().getRequiredPermissions())) {
                command.execute(new DBSUserCommandInteractionEvent(command, event));
